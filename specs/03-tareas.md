...
| 0.7 🔶 | X–J | Motor SENCE núcleo: `/api/sence/start`, `/api/sence/cb`, `/api/sence/close`, tablas `sence_sessions`+`sence_events`, máquina de estados, cifrado del token, traducción de errores | HU-5.1, 5.3, 5.4 | **Parcial (2026-07-14):** dominio puro (run/preflight/session), `errors.ts` 100–313, migración `sence_sessions`/`sence_events` con RLS y 180 tests verdes. **Falta:** rutas `/api/sence/*`, servicio con BD, cifrado AES del token, tenantGuard, suite de integración con BD |
