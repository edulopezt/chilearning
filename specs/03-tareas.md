...
- 5.1 🔶 Reproductor SCORM (spike con paquete Storyline real → integración scorm-again) — HU-4.2, ADR-006 — **hecho 2026-07-17** (ingesta #103, player #104): extracción en worker con presupuesto de bytes REAL en streaming (anti zip-bomb, `readEntryBytes`), proxy same-origin + CSP enforcing propia (D-052), score SCORM informativo (NUNCA a `grades`, D-050). Marcado 🔶 y no ✅: el spike con un paquete Storyline **real** (parte literal de esta tarea) NO se hizo — solo se probó contra un fixture sintético generado por código; sigue siendo handoff a Edu.
- 5.2 ✅ Portal empresa cliente + resumen semanal — HU-8.1, 8.2 — **hecho 2026-07-17** (#99): `companies`/`company_members` (1 empresa activa por usuario, D-062), rama `company` retirada de las policies vivas de `enrollments`/`sence_sessions` (D-061) — todo el acceso vía servicio auditado con RUN enmascarado. El resumen semanal (HU-8.2, redacción IA) se completó en la task 5.9.
- 5.3 ✅ Onboarding de tenant nuevo sin tocar código (criterio spec §8.4) + suspensión — HU-1.1, 1.4 — **hecho 2026-07-17** (#94): RPC `tenant_status_by_slug` + Auth Hook endurecido (tenant suspendido ⇒ claims sin roles), flags por feature (`scorm`/`ai_tutor`/`whatsapp`, deny-by-default) que habilitaron todo lo que vino después en el hito.
- 5.4 ✅ Sincrónico en vivo (lección tipo videoconferencia + asistencia RCE por sesión ⚠ validar norma) — spec §7-R3 — **hecho 2026-07-17** (#102), **alcance seguro aprobado por Edu en la planeación**: asistencia INTERNA por sesión (enlace externo Zoom/Meet/Teams), banner permanente de que no reemplaza el RCE, CERO cambios en `src/modules/sence/` (regla de re-entrada del piloto). La parte "asistencia RCE por sesión" del texto original de la tarea queda explícitamente PENDIENTE de que SENCE confirme la norma (D-051) — no es parte de lo entregado.
- 5.5 ✅ Tablero superadmin + métricas de negocio — HU-10.3 — **hecho 2026-07-17** (#95): RPC `platform_tenant_stats()` — se corrigió de SECURITY DEFINER a SECURITY INVOKER tras descubrir que en Supabase cloud (RLS forzada) el rol `postgres` NO bypassa RLS como en local; una versión DEFINER habría mostrado el tablero en ceros en producción.
- 5.6 🔶 Marca definitiva, dominio, landing comercial, política de privacidad y contrato de encargo revisados por abogado — Plan §13.3, §9 — **hecho 2026-07-17** (#97/#98): landing provisional "Chilearning" con párrafo de transparencia explícito (el RCE nunca ha corrido contra SENCE real), `/privacidad` con banner "BORRADOR", `docs/legal/CONTRATO-ENCARGO-BORRADOR.md`. Marcado 🔶 y no ✅: la revisión por **abogado** (parte literal de la tarea) NO ocurrió — sigue siendo handoff a Edu; la marca "Chilearning" es de trabajo, no definitiva.
...
- 5.8 ✅ Tutor IA (M11): RAG con pgvector, chat streaming, límites y panel de uso, derivación a humano — ADR-007 — **hecho 2026-07-18** (esquema #107, chat #108): retrieval híbrido lexical-primero/vector-cuando-hay-key (D-055, amplía ADR-007), minimización por construcción (`buildTutorPrompt` de firma lista blanca, staff sin lectura de conversaciones — D-056), reserva atómica de cupo de mensajes vía advisory lock tras un TOCTOU real cazado en revisión adversarial (D-058), costo real de OpenRouter vía RPC separada (D-059).
...
- 5.10 ✅ Creación asistida de cursos: asistente guiado paso a paso con dos entradas (desde cero o desde descriptor SENCE .docx) + plantillas por tipo — HU-3.5, 4.5 — **hecho 2026-07-17** (#105): módulos materializados como lección-cabecera (D-053). Nada se publica sin pasar por el constructor (todo queda en borrador). Se detectó y corrigió en una segunda pasada el mismo patrón insuficiente anti zip-bomb ya visto en SCORM, esta vez en el procesamiento del descriptor `.docx` (movido al worker con streaming real de bytes).
- 5.11 🔶 Canal WhatsApp operativo (plantillas aprobadas; envío directo worker→Meta, no en n8n — D-049, mismo criterio ya aceptado en 3.9/PR #66 y 5.9) — M9 — **hecho 2026-07-18** (#110): sender degradado (no-op sin credenciales Meta), plantillas versionadas `_v1`, opt-out independiente del de email (fix de una asimetría real cazada en revisión). Marcado 🔶 y no ✅: la verificación Meta Business sigue siendo trámite externo pendiente de Edu (docs/whatsapp/META-BUSINESS-VERIFICATION.md, task 3.10) y HOY ningún flujo puebla el teléfono del alumno (`user_metadata.phone`) — el canal queda cableado pero inalcanzable en la práctica hasta cerrar cualquiera de los dos puntos (ver `docs/whatsapp/ACTIVATION.md`).
