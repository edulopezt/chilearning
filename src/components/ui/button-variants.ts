...
// Separado de button.tsx (que es "use client" por el hook useDelayedValue):
// un archivo "use client" convierte TODAS sus exportaciones en referencias de
// cliente para Next — incluida una función cva pura sin hooks — así que los
// Server Components que solo necesitan las clases (landing, dashboard,
// privacidad) no pueden invocarla directamente si vive en el mismo módulo
// que el componente Button. Aquí queda sin "use client": server-safe.
const buttonVariants = cva(
  "group/button inline-flex shrink-0 cursor-pointer items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      // Escala táctil (RNF-6, mínimo 44px): "default"/"icon" son el tamaño primario y
      // cumplen el mínimo directamente. "xs"/"sm" quedan reservados a contextos densos
      // de escritorio (toolbars, filas de tabla) — nunca la única vía de una acción
      // primaria en una superficie táctil.
      size: {
        default:
