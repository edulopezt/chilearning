...
| Recurso | Valor |
|---|---|
| App staging | `https://seminarea.chilearning.cl` (login OK) |
| Tenant | Seminarea — `11111111-1111-4111-8111-111111111111` |
...
| URL de callback | `https://seminarea.chilearning.cl/api/sence/cb/<nonce>` (≤100 chars, OK) |
...
| Alumno demo (login) | user `aaaaaaaa-0000-4000-8000-000000000005` | `alumno@seminarea.test` |
| Admin OTEC (panel) | user `aaaaaaaa-0000-4000-8000-000000000001` | `admin@seminarea.test` |
...
- [ ] **Credenciales del alumno demo** (`alumno@seminarea.test`) y del **admin OTEC**
      (`admin@seminarea.test`) para loguearse en staging. *(Edu las tiene.)*
...
| URL de callback esperada | `https://seminarea.chilearning.cl/api/sence/cb/<nonce>` | staging |
...
4. Confirma que la app volvió a levantar: abre `https://seminarea.chilearning.cl` y verifica que
...
1. Loguéate en staging como **admin OTEC** (`admin@seminarea.test`).
2. Ve a `https://seminarea.chilearning.cl/admin/sence`.
...
1. Loguéate como el **alumno demo** (`alumno@seminarea.test`).
...
4. Confirma que los inputs ocultos `UrlRetoma` y `UrlError` son **exactamente**
   `https://seminarea.chilearning.cl/api/sence/cb/<nonce>` — con **`https://`**, host público, y
   ≤100 caracteres.
...
1. Loguéate en staging como el **alumno demo** (`alumno@seminarea.test`).
...
5. Al volver, SENCE postea el callback de **inicio exitoso** a
   `https://seminarea.chilearning.cl/api/sence/cb/<nonce>`:
